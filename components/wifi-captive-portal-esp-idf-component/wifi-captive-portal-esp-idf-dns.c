...
static void  dns_recv(struct sockaddr_in *premote_addr, char *pusrdata, unsigned short length) {
	...
	//Some sanity checks:
	if (length>WIFI_CAPTIVE_PORTAL_ESP_IDF_DNS_LEN) return; 								//Packet is longer than DNS implementation allows
	...
	if (hdr->flags&WIFI_CAPTIVE_PORTAL_ESP_IDF_DNS_FLAG_TC) return;								//truncated, can't use this
